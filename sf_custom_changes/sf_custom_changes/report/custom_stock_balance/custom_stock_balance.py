	...
	for company in sorted(iwb_map):
		for item in sorted(iwb_map[company]):
			for wh in sorted(iwb_map[company][item]):
				...
				data.append([item, item_map[item]["item_name"], item_map[item]["net_weight"],
					item_map[item]["weight_uom"],item_map[item]["stock_uom"],wh,qty_dict.opening_qty,
                                        qty_dict.in_qty,qty_dict.out_qty,
                                        qty_dict.bal_qty,item_map[item]["last_purchase_rate"],qty_dict.val_rate,
...
def get_columns(filters):
	...
	columns = ["Item:Link/Item:100", "Item Name::150", "Net Weight:Float:80", "Fill:Link/UOM:90", "Stock UOM:Link/UOM:90", \
        "Warehouse:Link/Warehouse:100", "Opening Qty:Float:100", "In Qty:Float:80", "Out Qty:Float:80", "Balance Qty:Float:100", \
...
def get_item_details(filters):
	item_map = {}
	for d in frappe.db.sql("select name, item_name, net_weight, stock_uom, weight_uom, last_purchase_rate, item_group, brand, \
