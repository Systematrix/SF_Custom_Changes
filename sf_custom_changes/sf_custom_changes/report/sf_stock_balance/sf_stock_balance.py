	...
	for company in sorted(iwb_map):
		for item in sorted(iwb_map[company]):
			for wh in sorted(iwb_map[company][item]):
				...
				data.append([item, item_map[item]["item_name"],wh,
					qty_dict.uom, qty_dict.opening_qty,qty_dict.in_qty,qty_dict.out_qty,
...
def get_columns(filters):
	...
	columns = ["Item:Link/Item:100", "Item Name::150", "Warehouse:Link/Warehouse:100", "Stock UOM::90", "Opening Qty:Float:100", \
        "In Qty:Float:80", "Out Qty:Float:80", "Balance Qty:Float:100", \
        "Valuation Rate:Float:90", "Opening Value:Float:110", "In Value:Float:80", \
