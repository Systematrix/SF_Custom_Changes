	...
	for item in sorted(iwb_map):
		for wh in sorted(iwb_map[item]):
			for batch in sorted(iwb_map[item][wh]):
                                for warehouse_lot_id in sorted(batch_map):
					qty_dict = iwb_map[item][wh][batch]
					data.append([item, item_map[item]["item_name"],
						item_map[item]["description"],
                                                wh,
                                                batch, 
                                                batch_map[warehouse_lot_id]["warehouse_lot_id"],
...
def get_columns(filters):
	...
	[_("Warehouse") + ":Link/Warehouse:100"] + [_("Batch") + ":Link/Batch:220"] + [_("Lot") + ":Link/Batch:220"] + \
        [_("Opening Qty") + "::90"] + \
...
def get_item_details(filters):
	...
	for d in frappe.db.sql("select name, item_name, description from tabItem", as_dict=1):
		item_map.setdefault(d.name, d)
                frappe.msgprint(item_map)

	return item_map

def get_batch_details(filters):
	batch_map = {}
	for d in frappe.db.sql("select name, item, warehouse_lot_id from tabBatch", as_dict=1):
		batch_map.setdefault(d.warehouse_lot_id, d)
                frappe.msgprint(batch_map)
